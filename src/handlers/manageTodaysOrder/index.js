...
exports.handler = async (event, context) => {
    try {
        ...
        let response = await service.manageTodaysOrders(parameter);
        ...
        if (response.length != 0) {
            ...
            return getResponseObject(
                ...
                HTTP_CODE.FAILURE,
                { isValid: true },
                "No manageTodaysOrders found for given User"
            );
        }
    } catch (error) {
        console.log("Error in manageTodaysOrders Detail handler: ", error);
