  ...
  try {
    ...
    if (parameter.reqtype == "s") {
      ...
      response.result = true,
        response.msg = "address inserted succesfully";

    } else if (parameter.reqtype == "u") {
      let result = await updateAddress(parameter);
      response.result = true,
        response.msg = "address updated succesfully";
...
const updateAddress = async (parameter) => {
  ...
  try {
    let query =
      `update TB_MSP_ADDRESS set AD_USERID = ?, AD_LINE1  = ?, AD_LINE2  = ?, AD_CITY = ?, AD_STATE = ?, AD_PIN = ?, AD_LAT = ?, AD_LON = ?, AD_UPD_DATE =? 
      where AD_ID = ?`;
    let result = await sequelize.query(query, {
      replacements: [
        parameter.userID,
...
const getMSPAddress = async (parameter) => {
  ...
  try {
    let query = "select AD_ID, AD_LINE1, AD_LINE2, AD_CITY, AD_STATE, AD_PIN, AD_REG_DATE, AD_LAT, AD_LON FROM TB_MSP_ADDRESS where AD_USERID = ?";
