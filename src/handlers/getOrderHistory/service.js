...
const userOrderDetails = async (parameter) => {
  ...
  //let t = await sequelize.transaction();
  let query = `SELECT A.O_ID, C.MSP_NAME, A.O_DISC, A.O_DISC_TYPE, A.O_TAKEWAY_CHG, A.O_DEL_CHG, A.O_FINAL_AMOUNT,A.O_STATUS, B.OD_MENUITEMS
      FROM TB_ORDER A
...
exports.mspOrderDetails = async (parameter) => {
  ...
  A.O_FINAL_AMOUNT,
  A.O_STATUS,
  MAX(B.OD_MENUITEMS) AS OD_MENUITEMS
FROM
  TB_ORDER A
JOIN
...
GROUP BY
  A.O_ID, U.U_FNAME, U.U_LNAME,UID, A.O_DISC, A.O_DISC_TYPE, A.O_TAKEWAY_CHG, A.O_DEL_CHG, A.O_FINAL_AMOUNT order by  A.O_ID desc;`;
  let results = await sequelize.query(query, {
