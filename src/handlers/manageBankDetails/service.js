const {
  ...
  const saveBankDetail = async (insertData) => {
    ...
    let query =
      ` INSERT INTO TB_BANKDETAILS ( B_USERID, B_BANKNAME, B_ACCOUNTNUMBER, B_ACCOUNTHOLDERNAME, B_IFSCCODE) 
      VALUES (?,?,?,?,?)`;
    let result = await sequelize.query(query, {
      replacements: [insertData.userID, insertData.bankName, insertData.accountNo, 
        insertData.accntName, insertData.ifsc,],
  ...
  const updateBankDetails = async (insertData) => {
    ...
    let query =
      ` UPDATE TB_BANKDETAILS SET B_BANKNAME = ?, B_ACCOUNTNUMBER = ?, B_ACCOUNTHOLDERNAME=?, 
