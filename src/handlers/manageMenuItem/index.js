...
exports.handler = async (event, context) => {
    try {
        ...
        let response = await service.manageMenuDetails(parameter);
        ...
        if (response.length != 0) {
            ...
            return getResponseObject(
                ...
                HTTP_CODE.FAILURE,
                { isValid: true },
                "No manageMenuDetails found for given User"
            );
        }
    } catch (error) {
        console.log("Error in manageMenuDetails Detail handler: ", error);
