...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        const result = await service.manageConsDetails(parameter);

        console.log('result ### ' + result)
        if (result.result == true) {
            return getResponseObject(true, HTTP_CODE.SUCCESS, { isValid: true }, result);

        } else return getResponseObject(false, HTTP_CODE.INTERNAL_SERVER_ERROR, { isValid: true }, result);
